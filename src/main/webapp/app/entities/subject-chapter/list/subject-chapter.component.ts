...
@Component({
  selector: 'jhi-subject-chapter',
  templateUrl: './subject-chapter.component.html',
})
export class SubjectChapterComponent {
 ...
 loadAllChaptersForASubject(selectedSubjectId: any): void {
        this.isLoading = true;
...
//  }
  ...
  trackId(index: number, item: ISubjectChapter): number {
    return item.id!;
  }
  getSubjectIdCallBack = (selectedSubjectId: any): void => {
	  // callback code here
	  this.selectedSubjectId=selectedSubjectId;
	  this.loadAllChaptersForASubject(selectedSubjectId);
 	 }
  ...
  delete(subjectChapter: ISubjectChapter): void {
    const modalRef = this.modalService.open(SubjectChapterDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.subjectChapter = subjectChapter;
  ...
  protected onSuccess(data: ISubjectChapter[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/subject-chapter'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
